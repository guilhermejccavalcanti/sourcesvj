...
public class JFSTMerge {
	...
	@Parameter(names = "-s", description = "Parameter to indicate the use of structured output instead of the semistructured one (true or false).")
	public static boolean isStructured = false;

	@Parameter(names = "-cl", description = "Parameter to count occurrence of consecutive lines conflict.", arity = 1)
	public static boolean countConsecutiveLinesConflicts = true;
	...
	public MergeContext mergeFiles(File left, File base, File right, String outputFilePath) {
		...
		boolean successssmerge 	= false;
		boolean successjdime 	= false;
		boolean successtextual 	= false;
		...
		if (FilesManager.areFilesDifferent(left, base, right, outputFilePath, context)) {
			...
			try {
				...
				if(JFSTMerge.mergeWithJdime){
					...
					context.structuredMergeTime = context.semistructuredMergeTime;
				}
				successjdime = true;
				...
				successssmerge = false;
				LOGGER.log(Level.WARNING, "", sme);
				context.semistructuredOutput 	= context.unstructuredOutput;
				context.semistructuredMergeTime = System.nanoTime() - t0un;
				context.structuredOutput 	= context.unstructuredOutput;
				context.structuredMergeTime = context.semistructuredMergeTime;

			} catch (StructuredMergeException ste) {
				successjdime = false;
				LOGGER.log(Level.WARNING, "", ste);
				context.structuredOutput 	= context.unstructuredOutput;
				context.structuredMergeTime = System.nanoTime() - t0un;

			}  finally {
				conflictState = checkConflictState(context);

				//FPFN assessing sucessfull and unsucessful merges, delete after
				context.sucessfullMerge = successjdime && successssmerge && successtextual;
				try{
					Statistics.computeSucessfullMerges(context,successssmerge,successjdime,successtextual);
				} catch(Exception e){
					System.err.println("An error occurred. See "+LoggerFactory.logfile+" file for more details.\n Send the log to gjcc@cin.ufpe.br for analysis if preferable.");
					LOGGER.log(Level.SEVERE,"",e);
					System.exit(-1);
		...
		if(context.sucessfullMerge){
			try {
				//FPFN
				FPFNUtils.countAndPrintStatsAndLogOfDifferences(context);
	...
	public static void main(String[] args) {
		...
		//				new JFSTMerge().mergeFiles(
		//						new File("E:/Pesquisa/tests-evosuite/maven/atmosphere/revisions/rev_41d97_95e31/left/modules/cpr/src/main/java/org/atmosphere/cpr/AtmosphereConfig.java"),
		//						new File("E:/Pesquisa/tests-evosuite/maven/atmosphere/revisions/rev_41d97_95e31/right/modules/cpr/src/main/java/org/atmosphere/cpr/AtmosphereConfig.java"),
		//						new File("E:/Pesquisa/tests-evosuite/maven/atmosphere/revisions/rev_41d97_95e31/base/modules/cpr/src/main/java/org/atmosphere/cpr/AtmosphereConfig.java"),
		//						null);

		//		new JFSTMerge().mergeRevisions("C:\\Users\\Guilherme\\Desktop\\tests-interference\\inter\\rev.revisions");

		//		new JFSTMerge().mergeRevisions("E:\\Pesquisa\\tests-evosuite\\maven\\Activiti\\revisions\\rev_0fb87_25e35\\rev.revisions");
		...
		//		new JFSTMerge().mergeFiles(
		//				new File("C:/Users/Guilherme/Google Drive/Pós-Graduação/Pesquisa/Outros/running_examples/examples_jdime/estudodecasofpfn/sem-interferencia-teste-abstract/left.java"),
		//				new File("C:/Users/Guilherme/Google Drive/Pós-Graduação/Pesquisa/Outros/running_examples/examples_jdime/estudodecasofpfn/sem-interferencia-teste-abstract/base.java"),
		//				new File("C:/Users/Guilherme/Google Drive/Pós-Graduação/Pesquisa/Outros/running_examples/examples_jdime/estudodecasofpfn/sem-interferencia-teste-abstract/right.java"),
		//				null);
	...
	private int checkConflictState(MergeContext context) {
		...
		if(JFSTMerge.isStructured) {
			...
			conflictList = FilesManager.extractMergeConflicts(context.semistructuredOutput); 
		}
