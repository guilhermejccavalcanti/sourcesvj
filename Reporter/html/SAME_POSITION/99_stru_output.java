...
public class StandardTitanTx extends TitanBlueprintsTransaction {
  ...
  public StandardTitanTx(StandardTitanGraph graph, TransactionConfiguration config, BackendTransaction txHandle) {
    ...
    indexCache = CacheBuilder.newBuilder().weigher(new Weigher<IndexQuery, List<Object>>() {
        @Override public int weigh(IndexQuery q, List<Object> r) {
          return 2 + r.size();
        }
  ...
  @Override public TitanVertex getVertex(final long vertexid) {
    verifyOpen();
  ...
  private class VertexConstructor implements Retriever<Long, InternalVertex> {
    ...
    @Override public InternalVertex get(Long vertexid) {
      ...
      if (idInspector.isTypeID(vertexid)) {
        ...
        if (idInspector.isVertexID(vertexid)) {
  ...
  @Override public TitanVertex addVertex(Long vertexId) {
    verifyWriteAccess();
  ...
  @Override public Iterable<Vertex> getVertices() {
    if (!addedRelations.isEmpty()) {
      ...
      while (viter.hasNext()){
        if (viter.next() instanceof TitanType) 
          viter.remove();
      }
  ...
  public final Object verifyAttribute(TitanKey key, Object attribute) {
    ...
    if (datatype.equals(Object.class)) {
      ...
      if (!attribute.getClass().equals(datatype)) {
        ...
        Preconditions.checkArgument(converted != null, "Value [%s] is not an instance of the expected data type for property key [%s] and cannot be converted. Expected: %s, found: %s", attribute, key.getName(), datatype, attribute.getClass());
        attribute = converted;
      }
  ...
  public void removeRelation(InternalRelation relation) {
    ...
    if (relation.isNew()) {
      ...
      if (isVertexIndexProperty(relation)) 
  ...
  @Override public TitanEdge addEdge(TitanVertex outVertex, TitanVertex inVertex, TitanLabel label) {
  ...
  private void connectRelation(InternalRelation r) {
    ...
    for (int pos = 0; pos < r.getLen(); pos++) 
      vertexCache.add(r.getVertex(pos), r.getVertex(pos).getID());
    if (isVertexIndexProperty(r)) 
      newVertexIndexEntries.add((TitanProperty)r);
  }
  @Override public TitanProperty addProperty(TitanVertex vertex, TitanKey key, Object value) {
  ...
  public TitanProperty addPropertyInternal(TitanVertex vertex, TitanKey key, Object value) {
    ...
    try {
      if (config.hasVerifyUniqueness()) {
        if (key.isUnique(Direction.OUT)) {
          Preconditions.checkArgument(Iterables.isEmpty(query(vertex).includeHidden().type(key).direction(Direction.OUT).properties()), "A property with the given key [%s] already exists on the vertex [%s] and the property key is defined as single-valued", key.getName(), vertex);
        }
        if (key.isUnique(Direction.IN)) {
          Preconditions.checkArgument(Iterables.isEmpty(getVertices(key, value)), "The given value [%s] is already used as a property and the property key [%s] is defined as graph-unique", value, key.getName());
        }
      }
  ...
  @Override public boolean containsType(String name) {
    verifyOpen();
  ...
  @Override public TitanType getType(String name) {
    verifyOpen();
    ...
    return (type != null) ? type : (TitanType)Iterables.getOnlyElement(getVertices(SystemKey.TypeName, name), null);
  }
  ...
  @Override public TitanKey getPropertyKey(String name) {
  ...
  @Override public TitanLabel getEdgeLabel(String name) {
    TitanType et = getType(name);
    if (et == null) {
  ...
  @Override public KeyMaker makeKey(String name) {
    StandardKeyMaker maker = new StandardKeyMaker(this, indexSerializer);
    maker.name(name);
    return maker;
  }
  @Override public LabelMaker makeLabel(String name) {
...
>>>>>>> C:\Users\user\Desktop\gjcc\amostra\projects\titan\revisions\rev_4935a97_c136530\rev_right_c136530\titan-core\src\main\java\com\thinkaurelius\titan\graphdb\transaction\StandardTitanTx.java
  ...
  public final QueryExecutor<GraphCentricQuery, TitanElement, JointIndexQuery> elementProcessor = new QueryExecutor<GraphCentricQuery, TitanElement, JointIndexQuery>() {
      private PredicateCondition<TitanKey, TitanElement> getEqualityCondition(Condition<TitanElement> condition) {
        if (condition instanceof PredicateCondition) {
          ...
          if (pc.getPredicate() == Cmp.EQUAL && isVertexIndexProperty(pc.getKey())) 
            return pc;
        }
        else 
      ...
      @Override public Iterator<TitanElement> getNew(final GraphCentricQuery query) {
        ...
        if (query.getResultType() == ElementType.VERTEX && hasModifications()) {
          ...
          if (query.getResultType() == ElementType.EDGE && !addedRelations.isEmpty()) {
      ...
      @Override public Iterator<TitanElement> execute(final GraphCentricQuery query, final JointIndexQuery indexQuery, final Object exeInfo) {
        ...
        if (!indexQuery.isEmpty()) {
          ...
          iter = Iterators.transform(resultSet.iterator(), new Function<Object, TitanElement>() {
              @Override public TitanElement apply(@Nullable Object id) {
  ...
  @Override public Iterable<TitanVertex> getVertices(TitanKey key, Object attribute) {
    Preconditions.checkNotNull(key);
    Preconditions.checkNotNull(attribute);
    return (Iterable)query().has(key, Cmp.EQUAL, attribute).vertices();
  }
  @Override public TitanVertex getVertex(TitanKey key, Object attribute) {
  ...
  @Override public TitanVertex getVertex(String key, Object attribute) {
    if (!containsType(key)) 
      return null;
    else 
      return getVertex((TitanKey)getType(key), attribute);
  }
  @Override public Iterable<TitanEdge> getEdges(TitanKey key, Object attribute) {
    Preconditions.checkNotNull(key);
    Preconditions.checkNotNull(attribute);
