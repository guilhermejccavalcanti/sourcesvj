...
public class PersistentIndex {
  ...
  public PersistentIndex(String datadir, Scheduler scheduler, Log log, StoreConfig config, StoreKeyFactory factory, MessageStoreRecovery recovery, StoreMetrics metrics) throws StoreException {
    try {
      ...
      journal = new InMemoryJournal(datadir, config.storeIndexMaxNumberOfInmemElements, config.storeMaxNumberOfEntriesToReturnFromJournal);
      ...
      for (int i = 0; i < indexFiles.length; i++) {
        ...
        logger.info("Index : {} loaded index segment {} with start offset {} and end offset {} ", datadir, indexFiles[i], info.getStartOffset(), info.getEndOffset());
      ...
      this.scheduler = scheduler;
      this.scheduler.schedule("index persistor", persistor, config.storeDataFlushDelaySeconds + new Random().nextInt(SystemTime.SecsPerMin), config.storeDataFlushIntervalSeconds, TimeUnit.SECONDS);
    }
    catch (Exception e) {
      throw new StoreException("Error while creating index " + datadir, e, StoreErrorCodes.Index_Creation_Failure);
    }
  }
  private void recover(IndexSegment segmentToRecover, long endOffset, MessageStoreRecovery recovery) throws StoreException, IOException {
    long startOffsetForRecovery = 0;
    if (segmentToRecover != null) {
      startOffsetForRecovery = segmentToRecover.getEndOffset() == -1 ? segmentToRecover.getStartOffset() : segmentToRecover.getEndOffset();
    }
    logger.info("Index : {} performing recovery on index with start offset {} and end offset {}", dataDir, startOffsetForRecovery, endOffset);
    ...
    for (MessageInfo info : messagesRecovered) {
      logger.trace("Index : {} recovering key {} offset {} size {}", dataDir, info.getStoreKey(), runningOffset, info.getSize());
      if (segmentToRecover == null) {
        segmentToRecover = new IndexSegment(dataDir, startOffsetForRecovery, factory, info.getStoreKey().sizeInBytes(), IndexValue.Index_Value_Size_In_Bytes, config, metrics);
      ...
      if (value != null) {
        ...
        if (value.getOriginalMessageOffset() != runningOffset && value.getOriginalMessageOffset() >= segmentToRecover.getStartOffset()) {
          journal.addEntry(value.getOriginalMessageOffset(), info.getStoreKey());
        }
        logger.info("Index : {} updated message with key {} size {} ttl {} deleted {}", dataDir, info.getStoreKey(), value.getSize(), value.getTimeToLiveInMs(), info.isDeleted());
      }
      else {
        ...
        journal.addEntry(runningOffset, info.getStoreKey());
        logger.info("Index : {} adding new message to index with key {} size {} ttl {} deleted {}", dataDir, info.getStoreKey(), info.getSize(), info.getExpirationTimeInMs(), info.isDeleted());
      }
  ...
  public void addToIndex(IndexEntry entry, FileSpan fileSpan) throws StoreException {
    ...
    if (needToRollOverIndex(entry)) {
      IndexSegment info = new IndexSegment(dataDir, entry.getValue().getOffset(), factory, entry.getKey().sizeInBytes(), IndexValue.Index_Value_Size_In_Bytes, config, metrics);
  ...
  public void addToIndex(ArrayList<IndexEntry> entries, FileSpan fileSpan) throws StoreException {
    ...
    if (needToRollOverIndex(entries.get(0))) {
      IndexSegment info = new IndexSegment(dataDir, entries.get(0).getValue().getOffset(), factory, entries.get(0).getKey().sizeInBytes(), IndexValue.Index_Value_Size_In_Bytes, config, metrics);
  ...
  private boolean needToRollOverIndex(IndexEntry entry) {
    return indexes.size() == 0 || indexes.lastEntry().getValue().getSizeWritten() >= maxInMemoryIndexSizeInBytes || indexes.lastEntry().getValue().getNumberOfItems() >= maxInMemoryNumElements || indexes.lastEntry().getValue().getKeySize() != entry.getKey().sizeInBytes() || indexes.lastEntry().getValue().getValueSize() != IndexValue.Index_Value_Size_In_Bytes;
  }
...
;
      for (Map.Entry<Long, IndexSegment> entry : indexes.entrySet()) {
        ...
        if ((fileSpan.getStartOffset() >= startOffset && fileSpan.getStartOffset() <= endOffSet) || (fileSpan.getEndOffset() >= startOffset && fileSpan.getEndOffset() <= endOffSet)) {
  ...
  protected IndexValue findKey(StoreKey key) throws StoreException {
    final Timer.Context context = metrics.findTime.time();
    try {
  ...
  public void markAsDeleted(StoreKey id, FileSpan fileSpan) throws StoreException {
    ...
    IndexValue newValue = new IndexValue(value.getSize(), value.getOffset(), value.getFlags(), value.getTimeToLiveInMs());
  ...
  public BlobReadOptions getBlobReadInfo(StoreKey id, EnumSet<StoreGetOptions> getOptions) throws StoreException {
    ...
    if (value == null) {
      ...
      if (value.isFlagSet(IndexValue.Flags.Delete_Index)) {
        ...
        if (value.isExpired() && !getOptions.contains(StoreGetOptions.Store_Include_Expired)) {
  ...
  public FindInfo findEntriesSince(FindToken token, long maxTotalSizeOfEntries) throws StoreException {
    try {
      ...
      if (storeToken.getSessionId() == null || storeToken.getSessionId().compareTo(sessionId) != 0) {
        if (!cleanShutdown) {
          if ((storeToken.getStoreKey() != null && storeToken.getIndexStartOffset() > logEndOffsetOnStartup) || (storeToken.getOffset() > logEndOffsetOnStartup)) {
      ...
      if (storeToken.getStoreKey() == null) {
        ...
        if (entries != null) {
          logger.trace("Index : " + dataDir + " retrieving from journal from offset " + offsetToStart + " total entries " + entries.size());
          ...
          for (JournalEntry entry : entries) {
            ...
            messageEntries.add(new MessageInfo(entry.getKey(), value.getSize(), value.isFlagSet(IndexValue.Flags.Delete_Index), value.getTimeToLiveInMs()));
          ...
          eliminateDuplicates(messageEntries);
          logger.trace("Index : " + dataDir + " new offset from find info" + " offset : " + (newToken.getOffset() != StoreFindToken.Uninitialized_Offset ? newToken.getOffset() : newToken.getIndexStartOffset() + ":" + newToken.getStoreKey()));
      ...
      throw new StoreException("Unknown error when finding entries for index " + dataDir, e, StoreErrorCodes.Unknown_Error);
    }
  }
  private long getTotalBytesRead(StoreFindToken newToken, List<MessageInfo> messageEntries, long logEndOffsetBeforeFind) {
  ...
  private StoreFindToken findEntriesFromOffset(long offset, StoreKey key, List<MessageInfo> messageEntries, long maxTotalSizeOfEntries) throws IOException, StoreException {
    IndexSegment segment = indexes.get(offset);
    IndexSegment lastSegment = indexes.lastEntry().getValue();
    if (segment.getStartOffset() == lastSegment.getStartOffset()) {
      throw new IllegalArgumentException("Index : " + dataDir + " findEntriesFromOffset from offset " + offset + " is in the last segment");
    }
    ...
    logger.trace("Index : " + dataDir + " findEntriesFromOffset from index start offset " + offset + " with key " + key + " total entries received " + messageEntries.size());
    ...
    while (currentTotalSizeOfEntries.get() < maxTotalSizeOfEntries && indexes.higherEntry(offset) != null){
      ...
      if (segment.getStartOffset() != lastSegment.getStartOffset()) {
        ...
        if (entries != null) {
          logger.trace("Index : " + dataDir + " findEntriesFromOffset from journal offset " + lastSegment.getStartOffset() + " total entries received " + entries.size());
          for (JournalEntry entry : entries) {
            offsetEnd = entry.getOffset();
            IndexValue value = findKey(entry.getKey());
            messageEntries.add(new MessageInfo(entry.getKey(), value.getSize(), value.isFlagSet(IndexValue.Flags.Delete_Index), value.getTimeToLiveInMs()));
          ...
          logger.trace("Index : " + dataDir + " findEntriesFromOffset from journal offset " + lastSegment.getStartOffset() + " offset not in journal");
        }
        break ;
      }
    }
    if (offsetEnd != StoreFindToken.Uninitialized_Offset) {
      ...
      return new StoreFindToken(messageEntries.get(messageEntries.size() - 1).getStoreKey(), lastSegmentIndex, sessionId);
    }
  }
  ...
  private List<MessageInfo> updateDeleteStateForMessages(List<MessageInfo> currentMessageEntries) throws StoreException {
    ...
    for (MessageInfo messageInfo : currentMessageEntries) {
      ...
      MessageInfo newMessageInfo = new MessageInfo(messageInfo.getStoreKey(), messageInfo.getSize(), indexValue.isFlagSet(IndexValue.Flags.Delete_Index), messageInfo.getExpirationTimeInMs());
  ...
  private void verifyFileEndOffset(FileSpan fileSpan) {
    if (getCurrentEndOffset() > fileSpan.getStartOffset() || fileSpan.getStartOffset() > fileSpan.getEndOffset()) {
      logger.error("File span offsets provided to the index does not meet constraints " + "logEndOffset {} inputFileStartOffset {} inputFileEndOffset {}", getCurrentEndOffset(), fileSpan.getStartOffset(), fileSpan.getEndOffset());
      throw new IllegalArgumentException("File span offsets provided to the index " + dataDir + " does not meet constraints" + " logEndOffset " + getCurrentEndOffset() + " inputFileStartOffset " + fileSpan.getStartOffset() + " inputFileEndOffset " + fileSpan.getEndOffset());
    }
  }
        ...
        if (indexes.size() > 0) {
          ...
          while (prevInfo != null && !prevInfo.isMapped()){
            if (prevInfo.getEndOffset() > currentLogEndPointer) {
              String message = "The read only index cannot have a file end pointer " + prevInfo.getEndOffset() + " greater than the log end offset " + currentLogEndPointer;
...
class StoreFindToken implements FindToken {
  ...
  @Override public byte[] toBytes() {
    int size = Version_Size + SessionId_Size + (sessionId == null ? 0 : sessionId.toString().getBytes().length) + Offset_Size + Start_Offset_Size + (storeKey == null ? 0 : storeKey.sizeInBytes());
