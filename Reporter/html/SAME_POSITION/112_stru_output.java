...
public abstract class FaunusElement implements Element, WritableComparable<FaunusElement> {
  ...
  protected static final Map<String, String> TYPE_MAP = new HashMap<String, String>() {
      @Override public final String get(final Object object) {
  ...
  public  <T extends java.lang.Object> T removeProperty(final String key) {
    return null == this.properties ? null : (T)this.properties.remove(key);
  }
  public  <T extends java.lang.Object> T getProperty(final String key) {
    if (key.equals(Tokens._COUNT)) 
      return 
...
;
      ...
      if (numberOfProperties == 0) 
        ...
        for (int i = 0; i < numberOfProperties; i++) {
          ...
          if (valueClass == PropertyType.INT.val) {
            ...
            if (valueClass == PropertyType.LONG.val) {
              valueObject = WritableUtils.readVLong(in);
            }
            else 
              if (valueClass == PropertyType.FLOAT.val) {
                valueObject = in.readFloat();
              }
              else 
                if (valueClass == PropertyType.DOUBLE.val) {
                  valueObject = in.readDouble();
                }
                else 
                  if (valueClass == PropertyType.STRING.val) {
                    valueObject = WritableUtils.readString(in);
                  }
                  else 
                    if (valueClass == PropertyType.BOOLEAN.val) {
  ...
  public static class Comparator extends WritableComparator {
    ...
    @Override public int compare(final byte[] element1, final int start1, final int length1, final byte[] element2, final int start2, final int length2) {
      try {
    ...
    @Override public int compare(final WritableComparable a, final WritableComparable b) {
