...
;
    ...
    final ListPreloader<Photo> preloader = new ListPreloader<Photo>(Glide.with(this), adapter, preloadSizeProvider, args.getInt(PRELOAD_KEY));
  ...
  @Override public void onPhotosUpdated(List<Photo> photos) {
  ...
  private class PhotoAdapter extends BaseAdapter implements ListPreloader.PreloadModelProvider<Photo> {
    ...
    @Override public List<Photo> getPreloadItems(int position) {
      return photos.subList(position, position + 1);
    }
    @Override public RequestBuilder getPreloadRequestBuilder(Photo item) {
