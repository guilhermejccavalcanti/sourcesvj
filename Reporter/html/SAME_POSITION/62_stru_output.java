...
;
  ...
  public void initialize(Configuration config) {
    try {
      ...
      String version = null;
      KCVSConfiguration systemConfig = new KCVSConfiguration(storeManager, SYSTEM_PROPERTIES_STORE_NAME, SYSTEM_PROPERTIES_IDENTIFIER);
      try {
  ...
  public final static KeyColumnValueStoreManager getStorageManager(Configuration storageConfig) {
    StoreManager manager = getImplementationClass(storageConfig, storageConfig.get(STORAGE_BACKEND), REGISTERED_STORAGE_MANAGERS);
    if (manager instanceof OrderedKeyValueStoreManager) {
      ...
      if (manager instanceof CacheStoreManager) {
  ...
  private final static Map<String, IndexProvider> getIndexes(Configuration config) {
    ...
    for (String index : config.getContainedNamespaces(INDEX_NS)) {
      ...
      IndexProvider provider = getImplementationClass(config.restrictTo(index), config.get(INDEX_BACKEND, index), REGISTERED_INDEX_PROVIDERS);
      Preconditions.checkNotNull(provider);
      builder.put(index, provider);
    }
    return builder.build();
  }
  public final static  <T extends java.lang.Object> T getImplementationClass(Configuration config, String clazzname, Map<String, String> registeredImpls) {
  ...
  public BackendTransaction beginTransaction(TransactionConfiguration configuration, KeyInformation.Retriever indexKeyRetriever) throws StorageException {
    ...
    if (!storeFeatures.supportsLocking()) {
      if (storeFeatures.supportsTxIsolation()) {
        ...
        if (storeFeatures.supportsConsistentKeyOperations()) {
          ...
          if (configuration.hasTimestamp()) 
            txConfig.setTimestamp(configuration.getTimestamp());
          tx = new ExpectedValueCheckingTransaction(tx, storeManager.beginTransaction(txConfig), readAttempts);
        }
    }
    ...
    return new BackendTransaction(tx, storeManager.getFeatures(), edgeStore, vertexIndexStore, edgeIndexStore, readAttempts, persistAttemptWaittime, indexTx, threadPool);
  }
  ...
  private final Function<String, Locker> CONSISTENT_KEY_LOCKER_CREATOR = new Function<String, Locker>() {
      @Override public Locker apply(String lockerName) {
  ...
  private final Function<String, Locker> ASTYANAX_RECIPE_LOCKER_CREATOR = new Function<String, Locker>() {
      @Override public Locker apply(String lockerName) {
        ...
        Preconditions.checkArgument(expectedManagerName.equals(actualManagerName), "Astyanax Recipe locker is only supported with the Astyanax storage backend (configured:" + actualManagerName + " != required:" + expectedManagerName + ")");
        try {
  ...
  private final Map<String, Function<String, Locker>> REGISTERED_LOCKERS = ImmutableMap.of("consistentkey", CONSISTENT_KEY_LOCKER_CREATOR, "astyanaxrecipe", ASTYANAX_RECIPE_LOCKER_CREATOR, "test", TEST_LOCKER_CREATOR);
