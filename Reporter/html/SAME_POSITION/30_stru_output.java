...
public class StandardTitanTx extends TitanBlueprintsTransaction implements TypeInspector, SchemaInspector, VertexFactory {
  ...
  public StandardTitanTx(StandardTitanGraph graph, TransactionConfiguration config) {
    ...
    indexCache = CacheBuilder.newBuilder().weigher(new Weigher<JointIndexQuery.Subquery, List<Object>>() {
        @Override public int weigh(JointIndexQuery.Subquery q, List<Object> r) {
          return 2 + r.size();
        }
  ...
  public StandardTitanTx getNextTx() {
    ...
    if (!config.isThreadBound()) 
      ...
      return (StandardTitanTx)graph.getCurrentThreadTx();
  }
  public TransactionConfiguration getConfiguration() {
    return config;
  }
  @Override public StandardTitanGraph getGraph() {
    return graph;
  }
  ...
  @Override public boolean containsVertex(final long vertexid) {
...
>>>>>>> C:\Users\user\Desktop\gjcc\amostra\projects\titan\revisions\rev_8889a15_80ac3f3\rev_right_80ac3f3\titan-core\src\main\java\com\thinkaurelius\titan\graphdb\transaction\StandardTitanTx.java
  ...
  @Override public Map<Long, TitanVertex> getVertices(long ... ids) {
    verifyOpen();
    ...
    for (java.util.Iterator<java.util.Map.Entry<java.lang.Long, @primitive.Unknown>> iterator = result.entrySet().iterator(); iterator.hasNext(); ) {
  ...
  private class VertexConstructor implements Retriever<Long, InternalVertex> {
    ...
    @Override public InternalVertex get(Long vertexid) {
  ...
  @Override public TitanVertex addVertex(Long vertexId, VertexLabel label) {
    verifyWriteAccess();
    if (label == null) 
  ...
  @Override public TitanVertex addVertex() {
    return addVertexWithLabel(BaseVertexLabel.DEFAULT_VERTEXLABEL);
  }
  @Override public TitanVertex addVertexWithLabel(String vertexLabel) {
    return addVertexWithLabel(getVertexLabel(vertexLabel));
  }
  @Override public TitanVertex addVertexWithLabel(VertexLabel vertexLabel) {
  ...
  private Iterable<InternalVertex> getInternalVertices() {
    ...
    if (!addedRelations.isEmpty()) {
      ...
      while (viter.hasNext()){
        if (viter.next() instanceof TitanSchemaElement) 
          viter.remove();
      }
    ...
    return Iterables.filter(allVertices, new Predicate<InternalVertex>() {
        @Override public boolean apply(@Nullable InternalVertex internalVertex) {
  ...
  public final Object verifyAttribute(PropertyKey key, Object attribute) {
    ...
    if (datatype.equals(Object.class)) {
      ...
      if (!attribute.getClass().equals(datatype)) {
        ...
        if (converted == null) 
          throw new SchemaViolationException("Value [%s] is not an instance of the expected data type for property key [%s] and cannot be converted. Expected: %s, found: %s", attribute, key.getName(), datatype, attribute.getClass());
        attribute = converted;
      }
  ...
  public void removeRelation(InternalRelation relation) {
    ...
    if (relation.isNew()) {
      ...
      if (TypeUtil.hasSimpleInternalVertexKeyIndex(relation)) 
        newVertexIndexEntries.remove((TitanProperty)relation);
    }
    else {
  ...
  @Override public TitanEdge addEdge(TitanVertex outVertex, TitanVertex inVertex, EdgeLabel label) {
  ...
  private void connectRelation(InternalRelation r) {
    ...
    for (int pos = 0; pos < r.getLen(); pos++) 
      vertexCache.add(r.getVertex(pos), r.getVertex(pos).getLongId());
    if (TypeUtil.hasSimpleInternalVertexKeyIndex(r)) 
      newVertexIndexEntries.add((TitanProperty)r);
  }
  @Override public TitanProperty addProperty(TitanVertex vertex, PropertyKey key, Object value) {
    if (key.getCardinality() == Cardinality.SINGLE) 
      return setProperty(vertex, key, value);
    else 
      ...
      if (config.hasVerifyUniqueness()) {
        ...
        if (cardinality == Cardinality.SET) {
          if (!Iterables.isEmpty(Iterables.filter(query(vertex).type(key).properties(), new Predicate<TitanProperty>() {
              @Override public boolean apply(@Nullable TitanProperty titanProperty) {
  ...
  public TitanProperty setProperty(TitanVertex vertex, final PropertyKey key, Object value) {
    ...
    Preconditions.checkNotNull(key);
    if (key.getCardinality() != Cardinality.SINGLE) 
      throw new UnsupportedOperationException("Not a single key: " + key + ". Use addProperty instead");
    TransactionLock uniqueLock = FakeLock.INSTANCE;
    try {
      if (config.hasVerifyUniqueness()) {
        ...
        for (InternalRelation r : v.it().getAddedRelations(new Predicate<InternalRelation>() {
            @Override public boolean apply(@Nullable InternalRelation p) {
  ...
  public final TitanSchemaVertex makeSchemaVertex(TitanSchemaCategory schemaCategory, String name, TypeDefinitionMap definition) {
    ...
    if (schemaCategory.isRelationType()) {
      ...
      if (schemaCategory == TitanSchemaCategory.VERTEXLABEL) {
    ...
    if (schemaCategory.hasName()) 
      newTypeCache.put(schemaCategory.getSchemaName(name), schemaVertex.getLongId());
    return schemaVertex;
  }
  ...
  public TitanSchemaVertex getSchemaVertex(String schemaName) {
    ...
    if (schemaId == null) 
      schemaId = graph.getSchemaCache().getSchemaId(schemaName, this);
    if (schemaId != null) {
  ...
  @Override public RelationType getExistingRelationType(long typeid) {
  ...
  @Override public VertexLabel getExistingVertexLabel(long id) {
  ...
  @Override public TitanMultiVertexQuery multiQuery(TitanVertex ... vertices) {
    MultiVertexCentricQueryBuilder builder = new MultiVertexCentricQueryBuilder(this);
    for (TitanVertex v : vertices) 
      builder.addVertex(v);
    return builder;
  }
  @Override public TitanMultiVertexQuery multiQuery(Collection<TitanVertex> vertices) {
    MultiVertexCentricQueryBuilder builder = new MultiVertexCentricQueryBuilder(this);
      ...
      for (TitanVertex v : vertices) {
        if (pos < vids.size() && vids.get(pos) == v.getLongId()) {
          ...
          ((CacheVertex)v).loadRelations(sq, new Retriever<SliceQuery, EntryList>() {
              @Override public EntryList get(SliceQuery query) {
                return vresults;
              }
          });
          pos++;
  ...
  public final QueryExecutor<VertexCentricQuery, TitanRelation, SliceQuery> edgeProcessorImpl = new QueryExecutor<VertexCentricQuery, TitanRelation, SliceQuery>() {
      @Override public Iterator<TitanRelation> getNew(final VertexCentricQuery query) {
        ...
        if (vertex.isNew() || vertex.hasAddedRelations()) {
          return (Iterator)vertex.getAddedRelations(new Predicate<InternalRelation>() {
              ...
              @Override public boolean apply(@Nullable InternalRelation relation) {
      ...
      @Override public boolean hasDeletions(VertexCentricQuery query) {
        return !deletedRelations.isEmpty() && !query.getVertex().isNew() && query.getVertex().hasRemovedRelations();
      }
      @Override public boolean isDeleted(VertexCentricQuery query, TitanRelation result) {
  ...
  public final QueryExecutor<GraphCentricQuery, TitanElement, JointIndexQuery> elementProcessorImpl = new QueryExecutor<GraphCentricQuery, TitanElement, JointIndexQuery>() {
      ...
      @Override public Iterator<TitanElement> getNew(final GraphCentricQuery query) {
        ...
        if (query.getResultType() == ElementCategory.VERTEX && hasModifications()) {
          ...
          if (standardIndexKey == null) {
            ...
            ConditionUtil.traversal(query.getCondition(), new Predicate<Condition<TitanElement>>() {
                @Override public boolean apply(@Nullable Condition<TitanElement> cond) {
            ...
            vertices = Iterators.transform(newVertexIndexEntries.get(standardIndexKey.getValue(), standardIndexKey.getKey()).iterator(), new Function<TitanProperty, TitanVertex>() {
                @Nullable @Override public TitanVertex apply(@Nullable TitanProperty o) {
                  return o.getVertex();
                }
            });
          }
          return (Iterator)Iterators.filter(vertices, new Predicate<TitanVertex>() {
              @Override public boolean apply(@Nullable TitanVertex vertex) {
                return query.matches(vertex);
              }
          });
        }
        else 
          if ((query.getResultType() == ElementCategory.EDGE || query.getResultType() == ElementCategory.PROPERTY) && !addedRelations.isEmpty()) {
      ...
      @Override public boolean hasDeletions(GraphCentricQuery query) {
        return hasModifications();
      }
      @Override public boolean isDeleted(GraphCentricQuery query, TitanElement result) {
      ...
      @Override public Iterator<TitanElement> execute(final GraphCentricQuery query, final JointIndexQuery indexQuery, final Object exeInfo) {
        ...
        if (!indexQuery.isEmpty()) {
          ...
          for (int i = 0; i < indexQuery.size(); i++) {
            ...
            retrievals.add(new QueryUtil.IndexCall<Object>() {
                @Override public Collection<Object> call(int limit) {
  ...
  private final Function<Object, TitanVertex> vertexIDConversionFct = new Function<Object, TitanVertex>() {
      @Override public TitanVertex apply(@Nullable Object id) {
  ...
  private final Function<Object, TitanEdge> edgeIDConversionFct = new Function<Object, TitanEdge>() {
      @Override public TitanEdge apply(@Nullable Object id) {
        Preconditions.checkNotNull(id);
  ...
  private final Function<Object, TitanProperty> propertyIDConversionFct = new Function<Object, TitanProperty>() {
      @Override public TitanProperty apply(@Nullable Object id) {
        Preconditions.checkNotNull(id);
        Preconditions.checkArgument(id instanceof RelationIdentifier);
        return ((RelationIdentifier)id).findProperty(StandardTitanTx.this);
      }
  };
  @Override public GraphCentricQueryBuilder query() {
    return new GraphCentricQueryBuilder(this, graph.getIndexSerializer());
  }
  @Override public TitanIndexQuery indexQuery(String indexName, String query) {
    return new IndexQueryBuilder(this, indexSerializer).setIndex(indexName).setQuery(query);
  }
  @Override public Iterable<TitanVertex> getVertices(PropertyKey key, Object attribute) {
    Preconditions.checkNotNull(key);
    Preconditions.checkNotNull(attribute);
    return (Iterable)query().has(key, Cmp.EQUAL, attribute).vertices();
  }
  @Override public Iterable<TitanEdge> getEdges(PropertyKey key, Object attribute) {
    Preconditions.checkNotNull(key);
    Preconditions.checkNotNull(attribute);
