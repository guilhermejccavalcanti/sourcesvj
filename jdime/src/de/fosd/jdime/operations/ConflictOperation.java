...
public class ConflictOperation<T extends Artifact<T>> extends Operation<T> {
    ...
    public void apply(MergeContext context) {
        ...
        if (context.isConditionalMerge(left) && leftCondition != null && rightCondition != null) {
			...
			//FPFN
            String content = conflict.prettyPrint();
            if(!content.contains("<<<<<<")){
            	if(left == null) context.mergeConflictsToFix.add(new FixedMergeConflict(content, FixedMergeConflict.DelectionDirection.LEFT));
            	else context.mergeConflictsToFix.add(new FixedMergeConflict(content, FixedMergeConflict.DelectionDirection.RIGHT));
            }
            
			//FPFN
			if(conflict instanceof ASTNodeArtifact){
				context.samePstCandidates.add((ASTNodeArtifact) conflict);
			}
        }
    }
