...
import jdime.fpfn.ASTBranchComparator;
...
public class StructuredStrategy extends MergeStrategy<FileArtifact> {
	...
	public void merge(MergeOperation<FileArtifact> operation, MergeContext context) {
		...
		try {
			...
			//FPFN
			/*			processSamePstCandidates(context);
			processSameStmtConflicts(context, base);*/
	...
	//FPFN
	private void processSamePstCandidates(MergeContext context) {
		for(ASTNodeArtifact conf : context.samePstCandidates){
		...
		context.samePstCandidates.clear();
	}

	//FPFN
	private void processSameStmtConflicts(MergeContext context,	ASTNodeArtifact base) {
		//FPFN
		ASTBranchComparator comparator = new ASTBranchComparator();
		comparator.context = context;
		context.editedBranches.EDITIONS_TO_DIFFERENT_PARTS_OF_SAME_STMT += comparator.countEditionsToDifferentPartsOfSameStmt(base.getRevision(), 
				context.editedBranches.getBranchesFromLeft(), 
				context.editedBranches.getBranchesFromRight());
	}
}
